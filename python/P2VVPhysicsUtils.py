###########################################################################################################################################
## P2VVPhysicsUtils: Physics utilities for P2VV                                                                                          ##
##                                                                                                                                       ##
## authors:                                                                                                                              ##
...
def convertLambda( config, fitResult, printToScreen = True ) :
  """converts the CP violation parameter lambda from a fit result with 'convertComplexPars'
  """
  ...
  if config.value('lambdaCPType') == 'polar' :
    ...
    polCovs = [ [ result[2][0][0], -result[2][0][1] ], [ -result[2][1][0], result[2][1][1] ] ]
    ...
    cartCovs = [ [ result[2][0][0], -result[2][0][1] ], [ -result[2][1][0], result[2][1][1] ] ]
...
def convertAmplitudes( config, fitResult, printToScreen = True ) :
  ...
  KSWave = False
  if config.value('KSWave') and type(config.value('KSWave')) is str and config.value('KSWave')[ :7 ] == 'include' : KSWave = True
  ...
  if config.value('ampsType') == 'transPolar' :
    ...
    polVals = [ ( result[1][0], 0. ), ( 1. - result[1][0] - result[1][2], result[1][1] ), ( result[1][2], result[1][3] ) ]
    if KSWave : polVals.append( ( result[1][4], result[1][5] ) )

    polCovs = [ list(row) for row in result[2] ]
    polCovs.insert( 1, [ 0. for i in range(len(result[2])) ] )                                   # A0Ph
    polCovs.insert( 2, [ -result[2][0][i] - result[2][2][i] for i in range(len(result[2])) ] )   # AparMag2
    for rowIter, row in enumerate(polCovs) :
      row.insert( 1, 0. )
      if rowIter == 1   : row.insert( 2, 0. )
      elif rowIter == 2 : row.insert( 2, result[2][0][0] + result[2][2][2] + 2. * result[2][0][2] )
      else              : row.insert( 2, polCovs[2][rowIter] )
    ...
    cartVals = [ ( 1., 0. ), ( result[1][0], result[1][1] ), ( result[1][2], result[1][3] ) ]
    if KSWave : cartVals.append( ( result[1][4], result[1][5] ) )
  ...
  if printToScreen :
    # print cartesian values and covariance matrix to screen
    params = [ ( 'Re(A_0)', 'Im(A_0)' ), ( 'Re(A_par)', 'Im(A_par)' ), ( 'Re(A_perp)', 'Im(A_perp)' ) ]
    if KSWave : params.append( ( 'Re(A_S)', 'Im(A_S)' ) )

    print 'P2VV - INFO: convertAmplitudes: cartesian amplitudes:'
    printComplexParams( params, cartVals, cartCovs )

    # print polar values and covariance matrix to screen
    params = [ ( '|A_0|^2', 'arg(A_0)' ), ( '|A_par|^2', 'arg(A_par)' ), ( '|A_perp|^2', 'arg(A_perp)' ) ]
    if KSWave : params.append( ( '|A_S|^2', 'arg(A_S)' ) )
...
def printComplexParams( parameters, values, covariances = None ) :
  ...
  for parIter, par in enumerate(parameters) :
    print '  {0:<11} = {1:10.4g} +/- {2:9.4g}'.format( par[0], values[parIter][0], sqrt(covariances[ 2*parIter ][ 2*parIter ]) )
    print '  {0:<11} = {1:10.4g} +/- {2:9.4g}'.format( par[1], values[parIter][1], sqrt(covariances[ 2*parIter + 1 ][ 2*parIter + 1 ]) )

  if covariances :
    # print covariance matrix
    print '\n  covariance matrix:\n              ',
    for par in parameters : print '{0:>11}  {1:>11} '.format( par[0], par[1] ),
    print

    for parIter0, par in enumerate(parameters) :
      print '  {0:<11} '.format(par[0]),
      for parIter1 in range(len(parameters)) :
        print '{0:11.3e}  {1:11.3e} '.format( covariances[ 2*parIter0 ][ 2*parIter1 ], covariances[ 2*parIter0 ][ 2*parIter1 + 1 ] ),
      print

      print '  {0:<11} '.format(par[1]),
      for parIter1 in range(len(parameters)) :
        print '{0:11.3e}  {1:11.3e} '.format(covariances[ 2*parIter0 + 1 ][ 2*parIter1 ], covariances[ 2*parIter0 + 1 ][ 2*parIter1 + 1 ]),
      print

  print


def convertComplexPars( values, covariances = None, inType = 'trans,cart', outType = 'trans,polar,magSq', normalize = 0 ) :
  ...
  if inHel == outHel and inPolar == outPolar and inMagSq == outMagSq and normalize < 1 : return (values, covariances)
  ...
  if covariances :
    ...
    covMatrixIn = TMatrixD( 2 * nPars, 2 * nPars, array( 'd', [ c_ij for c in covariances for c_ij in c[ : 2 * nPars ] ] ) )
  ...
  if normalize > 0 :
    ...
    norm = 0.
    for parIter in range(normalize) : norm += valsTemp[parIter][0] * valsTemp[parIter][0] + valsTemp[parIter][1] * valsTemp[parIter][1]
    norm = sqrt(norm)
  ...
  if outPolar :
    ...
    for parIter, par in enumerate(valsTemp) :
      ...
      if outMagSq : valsOut[parIter][0] = ASq
      else :        valsOut[parIter][0] = sqrt(ASq)
  ...
  if covariances :
    ...
    fullJacobian = TMatrixD( 2 * nPars, 2 * nPars, array( 'd', [ 1. if i == j else 0. for j in range(2*nPars) for i in range(2*nPars)] ) )
    ...
    covariancesOut = tuple( [ tuple( [ covMatrixOut[i][j] for j in range(2 * nPars) ] ) for i in range(2 * nPars) ] )
