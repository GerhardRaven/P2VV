...
void defineObservables(RooWorkspace& ws)
{
  ...
  obs.add(*new RooRealVar("t","B proper time",-0.5,10)) ;
  ...
  obs.add(*new RooRealVar("nPV","nPV",0,100)) ;
  obs.add(*new RooRealVar("nTrack","nTrack",0,10000)) ;
  obs.add(*new RooRealVar("ipchi2next","ipchi2next",50,1e6)) ;
  obs.add(*new RooRealVar("TCK","TCK",0,1e7)) ;
  ...
  ws.defineSet("observables","m,t,sigmat,mdau1,mdau2,pt,chi2dof,decaytype,"\
	       "mKK,mKS,trcostheta,trcospsi,trphi,nPV,nTrack,ipchi2next,TCK") ;
...
{  
  ...
  //ws.factory("GaussModel::tres_sig_g1(t,tres_sig_m1[0,-0.2,0.2], sigmat, const_one, tres_sig_s1[1.1,0.3,2])");
  //ws.factory("GaussModel::tres_sig_g2(t,tres_sig_m1,             sigmat, const_one, tres_sig_s2[2.0,1.5,10])");
  ...
  ws.factory("AddModel::tres_sig({tres_sig_g2,tres_sig_g1},{tres_sig_f2[0.2,0.01,1]})");
  ...
  //ws.factory("GaussModel::tres_bkg_g1(t,tres_bkg_m1[0,-0.2,0.2], sigmat, const_one, tres_bkg_s1[1.5,0.3,3])");
  //ws.factory("GaussModel::tres_bkg_g2(t,tres_bkg_m1,             sigmat, const_one, tres_bkg_s2[3.0,0.9,10])");
  ...
  ws.factory("AddModel::tres_bkg({tres_bkg_g2,tres_bkg_g1},{tres_bkg_f2[0.2,0.01,1]})");
...
RooWorkspace* createWS(DecayMode mode, const char* title = "", const char* selection = 0 )
{
  ...
  tree->Draw(">>evtlist",selection) ;
  evtlist = dynamic_cast<TEventList*>(gDirectory->Get("evtlist")) ;
  tree->SetEventList(evtlist) ;
  TFile* f = new TFile("/tmp/tmp.root","RECREATE") ;
  TTree* subtree = tree->CopyTree("");
  //TTree* subtree = tree ;

  //tree->Scan("m","","",5) ;
  RooDataSet* data = new RooDataSet("data",title,subtree,*ws->set("observables"),
...
void fit(DecayMode mode, const char* title = "", const char* selection = 0, bool yieldsonly=false) 
{
  ...
  if(yieldsonly) {
    setConstant(*ws,"*",true) ;
    setConstant(*ws,"N*",false) ;
    //setConstant(*ws,"tres_*_f2",false) ;
    //ws->var("tres_sig_f3")->setVal(0) ;
    //ws->var("tres_bkg_f3")->setVal(0) ;
  }
...
void plot( RooWorkspace& ws)
{
 ...
 // ===========================================================================================================
  ...
  c->Print(TString(ws.cat("decaymode")->getLabel()) + data->GetTitle() + "Fit.eps") ;
...
void makeMassSPlots( DecayMode mode, const char* selection=0, bool refit = false )
{
  RooWorkspace* ws = gMyWorkspace ;
  ...
  // - 0.02ps bins on [-0.2,0.2]
  // - 0.5 ps for the rest
  double tmin = t->getMin() ;
  double tmax = t->getMax() ;
  std::set<double> setbins ;
  for( double t = tmin; t<tmax; t += 0.5  ) setbins.insert(t) ;
  for( double t = -0.2; t<=0.2; t += 0.02 ) setbins.insert(t) ;
  setbins.insert(tmax) ;
  std::vector<double> vecbins(setbins.begin(),setbins.end()) ;
  t->setBinning(RooBinning(vecbins.size()-1,&(vecbins.front()),"VarBinning")) ;
  t->setBins(1100) ;
  ...
  histdata  = new RooDataHist("wtsigmat","",RooArgSet(*t,*sigmat),*tmpdata,"VarBinning") ;
  sigmat->setVal(0.05) ;
  ...
  RooPlot* psiwtframe = t->frame(Bins(nbins),Range(tmin,tmax));
  ...
  RooPlot* bkgwtframe = t->frame(Bins(nbins),Range(tmin,tmax));
  ...
  RooPlot* sigwtframe = t->frame(Bins(42),Range(-0.5,10));
  histdata = new RooDataHist("wtsigmat","",RooArgSet(*t,*sigmat),*tmpdata,"VarBinning") ;
...
void writeSTuple( DecayMode mode, bool massonly=false )
{
  RooWorkspace* ws = gMyWorkspace ;
  if(ws == 0) ws = createWS(mode) ;
  ...
  std::cout << "Making splot." << std::endl ;

  // create an splot
  RooStats::SPlot* splot = 
    new RooStats::SPlot("splotdata","splotdata",*data,pdf,*(ws->set("yields"))) ;

  std::cout << "Writing data set to file." << std::endl ;
  // now save the data set
  RooDataSet* wdata = splot->GetSDataSet() ;
  
  TString name = TString(ws->cat("decaymode")->getLabel()) ;
  if(massonly) name += "M" ;

  TFile file( TString("$HOME/w0/") + name + "SPlots.root","RECREATE") ;
  wdata->tree()->Write("dataset") ;
  file.Write() ;
  file.Close() ;
  
  std::cout << "Written dataset to file: " << file.GetName() << std::endl ;

}

void makeSPlots( DecayMode mode, 
		 const char* varname, int nbins, double xmin, double xmax )
{
  RooWorkspace* ws = gMyWorkspace ;
  if(ws == 0) ws = createWS(mode) ;
  ...
  if(var==0) {
    std::cout << "Cannot find variable with name: " << varname << std::endl ;
    return ;
  }
  ...
  c1->cd(1) ;
  tmpdata = new RooDataSet(wdata->GetName(),wdata->GetTitle(),wdata,*wdata->get(),0,"Nsig_sw") ;
  frame = var->frame(Bins(nbins),Range(xmin,xmax)) ;
  tmpdata->plotOn( frame,DataError(RooAbsData::SumW2) ) ;
  frame->SetTitle(TString(varname) + " splot for signal") ;
  frame->Draw() ;
  ...
  frame = var->frame(Bins(nbins),Range(xmin,xmax)) ;
  tmpdata->plotOn( frame,DataError(RooAbsData::SumW2) ) ;
  frame->SetTitle(TString(varname) + " splot for prompt psi") ;
  frame->Draw() ;
  ...
  frame = var->frame(Bins(nbins),Range(xmin,xmax)) ;
  tmpdata->plotOn( frame,DataError(RooAbsData::SumW2) ) ;
  frame->SetTitle(TString(varname) + " splot for background") ;
  frame->Draw() ;
