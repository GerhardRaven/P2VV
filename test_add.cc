...
RooAbsReal& prod(RooWorkspace& w,const char* a, const char *b=0, const char *c=0, const char *d=0) {
...
{ 
        ...
        RooArgSet amp = w.argSet("ReAz,ImAz,ReAperp,ImAperp,ReApar,ImApar,qtag,C,AzAz_basis,AperpAperp_basis,AparApar_basis,AparAperp_basis,AzAperp_basis,AzApar_basis");
        import(w,RooFormulaVar("NAzAz",       "@8 * ( @0 * @0 + @1 * @1 ) / ( 1+@6*@7 )", amp));
        import(w,RooFormulaVar("NAparApar",   "@10* ( @4 * @4 + @5 * @5 ) / ( 1+@6*@7 )", amp));
        import(w,RooFormulaVar("NAperpAperp", "@9 * ( @2 * @2 + @3 * @3 ) / ( 1+@6*@7 )", amp));
        import(w,RooFormulaVar("ReAparAperp", "@11* ( @4 * @2 + @5 * @3 ) / ( 1+@6*@7 )", amp));
        import(w,RooFormulaVar("ReAzAperp",   "@12* ( @0 * @2 + @1 * @3 ) / ( 1+@6*@7 )", amp));
        import(w,RooFormulaVar("ReAzApar",    "@13* ( @0 * @4 + @1 * @5 ) / ( 1+@6*@7 )", amp));
        import(w,RooFormulaVar("ImAparAperp", "@11* ( @4 * @3 - @5 * @2 ) / ( 1+@6*@7 )", amp));
        ...
        import(w, RooAddition_("f_cosh","f_cosh",RooArgSet( prod(w, "NAzAz"                 )
                                                          , prod(w, "NAparApar"             )
                                                          , prod(w, "NAperpAperp"           )
                                                          , prod(w, "ImAparAperp","qtag_","C")
                                                          , prod(w, "ImAzAperp",  "qtag_","C")
                                                          , prod(w, "ReAzApar"              ) )));
        import(w, RooAddition_("f_cos" ,"f_cos", RooArgSet( prod(w, "NAzAz",    "qtag_","C")
                                                          , prod(w, "NAparApar",  "qtag_","C")
                                                          , prod(w, "NAperpAperp","qtag_","C")
                                                          , prod(w, "ImAparAperp"           )
                                                          , prod(w, "ImAzAperp"             )
                                                          , prod(w, "ReAzApar",   "qtag_","C") )));
        import(w, RooAddition_("f_sinh","f_sinh",RooArgSet( prod(w, "NAzAz",      "Minus",       "D")
                                                          , prod(w, "NAparApar",  "Minus",       "D")
                                                          , prod(w, "NAperpAperp",               "D")
                                                          , prod(w, "ReAparAperp",        "qtag_","S")
                                                          , prod(w, "ReAzAperp",          "qtag_","S")
                                                          , prod(w, "ReAzApar",   "Minus",       "D") )));
        import(w, RooAddition_("f_sin" ,"f_sin", RooArgSet( prod(w, "NAzAz",       "Minus","qtag_","S")
                                                          , prod(w, "NAparApar",   "Minus","qtag_","S")
                                                          , prod(w, "NAperpAperp",         "qtag_","S")
...
void test_add() {
    ...
    //RooMsgService::instance().addStream(RooFit::DEBUG,Topic(RooFit::Integration));
