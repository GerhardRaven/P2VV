\documentclass[a4paper,9pt,twosided]{article}
\usepackage{rotating}
\usepackage{amsmath}
...
By using the observation that $\sin(\Delta m t) = \Im\left( e^{i\Delta m t}\right)$ and $\cos(\Delta m t ) = \Re\left( e^{ i \Delta m t } \right)$, and that both the $\cosh$ and $\sinh$ can
be written as the sum and difference of exponentials, i.e. the same as the $\cos$ term with $\Delta m = 0$, it is sufficient to consider the following convolution:
\begin{eqnarray*}
...
\end{eqnarray*}
Finally, we use the Faddeeva function $w$,  defined as 
\begin{equation}
    w(z) = e^{-z^2}\mathrm{erfc}\left(-iz\right)
\end{equation}
and implemented in CERNLIB (see http://wwwasdoc.web.cern.ch/wwwasdoc/shortwrupsdir/c335/top.html) as $cwerf$, to write the result as:
\begin{eqnarray*}
...
\begin{equation*}
    \int dt f(t;\Gamma,\Delta m, \sigma) = 
\end{equation*}

where we use that
\begin{equation}
   I_0(x_\mathrm{min},x_\mathrm{max};z)\equiv \int_{x_\mathrm{min}}^{z_\mathrm{max}} dx e^{-\frac{1}{2}x^2} w\left( \frac{i}{\sqrt{2}}(z-x) \right) 
              = \frac{1}{z}\left[ \mathrm{erf}\left(\frac{x}{\sqrt{2}} \right) - w\left( \frac{i}{\sqrt{2}}\left(z-x\right) \right) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
\end{equation}
...
the integrals of $t^n f(t)$ for $n=1,2,3$ are needed for the normalization.
...
\begin{eqnarray*}
       I(x_\mathrm{min},x_\mathrm{max};z,\lambda) &=&  \int_{x_\mathrm{min}}^{x_\mathrm{max}} dx\; e^{\lambda x} e^{-\frac{1}{2}x^2} w\left( \frac{i}{\sqrt{2}}(z-x)\right) 
        ...
        \\            &=& \frac{e^{\frac{1}{2}\lambda^2}}{z-\lambda} \left[ \mathrm{erf}\left( \frac{x}{\sqrt{2}} \right) - w\left(\frac{i}{\sqrt{2}}\left(z-\lambda-x \right) \right) \right]_{x_\mathrm{min}-\lambda}^{x_\mathrm{max}-\lambda}
        \\            &=& e^{\frac{1}{2}\lambda^2} \left[  J(x_\mathrm{max};\lambda,z) - J(x_\mathrm{min};\lambda,z)  \right]
\end{eqnarray*}
where $J(x;\lambda,z)=\frac{1}{z-\lambda}\left[\mathrm{erf}\left(\frac{x-\lambda}{\sqrt{2}}\right)-w\left(\frac{i}{\sqrt{2}}\left(z-x\right) \right)\right]$.

In order to simplify the  computation of  $I_n$, we define $K_n,L_n(z)$ and $M_n(x;z)$ as follows:
\begin{equation}
\begin{array}{|c|c|c|c|c|c|}
\hline
n   
& \begin{array}{l} K_n     \\ = \left. \left( \frac{d}{d\lambda} \right)^n\right|_{\lambda=0} e^{\frac{1}{2}\lambda^2}  \end{array}
& \begin{array}{l} L_n(z)  \\ = \left. \left( \frac{d}{d\lambda} \right)^n\right|_{\lambda=0} \frac{1}{z-\lambda}\\ =n!z^{-n-1} \end{array}
& \begin{array}{l}  M_n(x;z)  \\ = \left. \left( \frac{d}{d\lambda} \right)^n\right|_{\lambda=0} \left[ \mathrm{erf}\left(\frac{x-\lambda}{\sqrt{2}}\right)-w\left(\right) \right] \end{array}
\\ \hline 0  & 1 & z^{-1} &  \mathrm{erf}\left(\frac{x}{\sqrt{2}}\right)-w\left( \frac{i}{\sqrt{2}} \left(z-x\right)\right) 
\\ 1  & 0 & z^{-2} & -  \sqrt{\frac{2}{\pi}}e^{-\frac{x^2}{2}}
\\ 2  & 1 & 2 z^{-3} & x M_1(x;z)
\\ 3  & 3 & 6 z^{-4} & (x^2-1)M_1(x;z)
\\ \hline
\end{array}
\end{equation}

In terms of these functions, the normalization integrals can be written as:
\begin{eqnarray}
     I_0(x_\mathrm{min},x_\mathrm{max};z) &=&  z^{-1} \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
\\   I_1(x_\mathrm{min},x_\mathrm{max};z) &=& %   K_1 L_0(z) \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                                  z^{-2} \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\  &+&   z^{-1} \left[ M_1(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
\\   I_2(x_\mathrm{min},x_\mathrm{max};z) &=&     z^{-1} \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+&  2 z^{-3} \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+&    z^{-1} \left[ x M_1(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+&  2 z^{-2} \left[ M_1(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
\\   I_3(x_\mathrm{min},x_\mathrm{max};z) &=&   3 z^{-1} \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+&  6 z^{-4} \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+&    z^{-1} \left[ (x^2-1)M_1(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+& 3  z^{-2} \left[ M_0(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+& 6  z^{-3} \left[ M_1(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+& 3  z^{-2} \left[ xM_1(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
                                        \\ &+& 3  z^{-1} \left[ M_1(x;z) \right]_{x_\mathrm{min}}^{x_\mathrm{max}}
\end{eqnarray}
where the values of $K_n$ and $L_n(z)$ have already been substituted, and the expressions for $M_n$ with $n>1$ have been written in terms of $M_1$.
\pagebreak
We give here the expressions for $n=1,2$:
\begin{equation*}
    I_1(x,z)\equiv \int dx\; x e^{-\frac{1}{2}x^2} w\left( \frac{i}{\sqrt{2}}(z-x)\right) 
    = \frac{1}{z}\left[I_0(z,x)-\sqrt{\frac{2}{\pi}}e^{-\frac{1}{2}x^2} - x w\left( \frac{i}{\sqrt{2}}(z-x)\right)  \right]
\end{equation*}
\begin{equation*}
    I_2(x,z)\equiv \int dx\; x^2 e^{-\frac{1}{2}x^2} w\left( \frac{i}{\sqrt{2}}(z-x)\right) 
    = \frac{1}{z}\left[ 2I_1(x,z) - x^2z^2 w\left(\right) \right]
\end{equation*}
