...
To describe a non-trivial decaytime efficiency by taking the product with cubic b-splines, one encounters functions of the shape:
\begin{eqnarray*}
     g(t) &=& (t-\alpha_i)(t-\beta_i)(t-\gamma_i) f(t)
\\        &=& \left( t^3 - (\alpha+\beta+\gamma)t^2 + (\beta\gamma+\alpha\gamma+\alpha\beta)t - \alpha\beta\gamma \right) f(t)
\end{eqnarray*}
Hence one needs to compute integrals of the form:
\begin{eqnarray*}
  \int dt g(t;)  &=& \int dt \left( t^3 - (\alpha+\beta+\gamma)t^2 + (\beta\gamma+\alpha\gamma+\alpha\beta)t - \alpha\beta\gamma \right) f(t;\Gamma,\Delta m,\sigma)
