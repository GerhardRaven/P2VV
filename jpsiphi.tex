...
The complete spline function $ \mathcal{S}(u)$ is given by the linear combination $\mathcal{S}(u)= \sum_{i=1}^{n+3} b_i  \mathcal{S}_i(u)$.
For a given knot vector, in the $i^\mathrm{th}$ interval, i.e. for $u_{i-1}<  u\le u_i$, there are (upto) four b-splines contributing. To
compute their contribution, we extend the knot vector at both ends by three entries given by  
\begin{eqnarray*}
 u_{-3}=u_{-2}=u_{-1}&=&u_0
