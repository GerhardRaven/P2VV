...
\end{equation}
where we have assumed that the {\em combination} of $P_{B}+P_{\overline{B}}$ is properly normalized, as in that
case also the derived $P_{B_\mathrm{tag},i}, P_{\overline{B}_\mathrm{tag},i}$ and $P_{\mathrm untagged}$ are properly normalized, because:
\begin{equation}
     \sum_i \left( P_{B_\mathrm{tag},i}+P_{\overline{B}_\mathrm{tag},i} \right) + P_{\mathrm untagged} = P_B + P_{\overline B}
\end{equation}
by construction.


Taking into account that $P_{B}$ and $P_{\overline{B}}$ can differ in overall normalization
due to an explicit production asymmetry, or due to a nonzero value of $C$, which results in
...
\begin{eqnarray}
   ...
   P(q_T=-1,i) &=& t_i w_i\frac{{\cal E}+{\cal O}}{N_+} + \overline{t}_i(1-\overline{w}_i)\frac{{\cal E} - {\cal O}}{N_-} 
...
At this point, let us look once more at the overall normalization. Performing the sum over all tagging categories, and for
the tagged events, over $q_T\in(-1,1)$, and adding the untagged events,
and integrating over the decay time yields the following overall normalization:
\begin{equation}
    N = I_{\cal E}  + 2 \nu I_{\cal O} 
\label{eq:overallNorm}
\end{equation}
where we have defined $I_{\cal E} = \int dt {\cal E}(t)$ and $I_{\cal O} = \int dt {\cal O}(t)$.

The PDFs for tagged events can now be written as:
\begin{eqnarray}
...
\\             &+&\frac{1}{2} \epsilon_i(1+\nu)(1+\delta_i)\left(1-{\cal D}_i(1-{\cal W}_i)\right)\left({\cal E}+{\cal O}\right)
\end{eqnarray}
where we have omitted(!) the overall normalization constant $N$, defined by \ref{eq:overallNorm}, which is shared by each term. 
...
\\         &-& \epsilon_i\left[ (1+\delta_i\nu)     {\cal D}_i              -  (\delta_i+\nu)  (1+{\cal D}_i{\cal W}_i ) \right]{\cal O}
\\   P(q_T=0)  
           &=&           \left[ 1- \sum_i \epsilon_i (1+\delta_i\nu) \right] {\cal E} \nonumber
\\         &+&           \left[ 2\nu - \sum_i \epsilon_i(\nu+\delta_i) \right] {\cal O} \label{eq:untagged}
\end{eqnarray}
If instead we collect all CP even and CP odd terms together, we find for the tagged events:
\begin{eqnarray}
     P(q_T\in (-1,+1),i)  &=& \epsilon_i\left[ (1+\delta_i\nu){\cal E} + (\delta_i+\nu){\cal O} \right]
...
\end{eqnarray}
The other direction one can take is to marginalize over the tagging decision, i.e. take the sum over $q_T=-1$  and
$q_T=+1$. This yields 
\begin{eqnarray}
     P(i)  &=& \epsilon_i\left[ (1+\delta_i\nu){\cal E} + (\delta_i+\nu){\cal O} \right]
\end{eqnarray}
which is indeed the complement of the untagged events, defined by eq.\ref{eq:untagged}: when adding all these PDFs,
one finds again back something which is consistent with the normalization found in eq. \ref{eq:overallNorm}
...
\begin{eqnarray}
   {\cal E} &\rightarrow& {\cal E} \left[ A_i +  q_T {\cal D}_i \left( B_i - {\cal W}_i A_i \right) \right]  \\
   {\cal O} &\rightarrow& {\cal O} \left[ B_i +  q_T {\cal D}_i \left( A_i - {\cal W}_i B_i \right) \right].
\end{eqnarray}
