...
Note that we have not assumed that $|\lambda|=1$, but we did assume that all three $|\lambda_i|$ are the same, i.e.
...
i.e. $|\overline{A}_i/A_i|$ are all the same. The above does allow for CP violation in mixing, as in that case $|q/p|\ne 1$,
and all three $|\lambda_i|$ would be, by definition affected identically. As a result, {\em assuming the absence of direct CP},
...
i.e. the quantities $\nu,\delta_i,{\cal D}_i$ are CP odd, whereas $N,\epsilon_i,{\cal W}_i$ are CP even.
...
terms have to be kept\footnote{Keeping all terms is left as an exercise to the reader. It should be
noted that given their properties under CP, there is little choice in where those terms can/will appear}. This results in:
\begin{eqnarray}
...
\begin{eqnarray}
     P(q_T,i)  % &=& \epsilon_i\left(1            +q_T{\cal D}(  \nu+\delta_i-{\cal W}) \right){\cal E} \\
               % &+& \epsilon_i\left( \nu+\delta_i+q_T{\cal D}  \right){\cal O} \\
               &=& \epsilon_i\left[\left({\cal E}+(\nu+\delta_i){\cal O})  \right) + q_T {\cal D}_i \left({\cal O}+(\nu+\delta_i-{\cal W}_i ){\cal E} \right)\right]
\end{eqnarray}
and the (flavour) asymmetry can be written as:
\begin{equation}
  {\cal A}(i) =  {\cal D}_i \frac{{\cal O}+(\nu+\delta_i-{\cal W}_i ){\cal E}}
                               {{\cal E}+(\nu+\delta_i){\cal O} }
\end{equation}
