...
\begin{eqnarray*}
     A''_i(u) &=& -6\frac{\Delta_{i+1}(u)}{P_i}  
\\   B''_i(u) &=& 
...
\begin{equation}
   G_i = \left(  \begin{array}{cccc}
       ...
       \end{array} \right)
\end{equation}
For this we need a large number of integrals of quadratic combinations of $\Delta_j(u)\Delta_k(u)$. 
Fortunately, using partial integration, one can write generically
\begin{eqnarray*}
   I_i^{jk}  \equiv  \int_{u_i}^{u_{i+1}} du \Delta_j(u) \Delta_k(u) 
      &=& \left[ \frac{1}{2} \Delta_j(u)^2 \Delta_k(u) \right]_{u_i}^{u_{i+1}} - \left[  \frac{1}{6} \Delta_j(u)^3 \right]_{u_i}^{u_{i+1}} 
\\    &=& \frac{1}{6}\left[ \Delta_j(u)^2(3\Delta_k(u)-\Delta_j(u) )\right]_{u_i}^{u_{i+1}} 
\\    &=& \frac{1}{6}\left[ (\Delta_j^{i+1})^2(3\Delta_k^{i+1}-\Delta_j^{i+1} )
                          - (\Delta_j^i)^2(3\Delta_k^i-\Delta_j^i )\right]
\end{eqnarray*}

\begin{eqnarray*}
          \int_{u_i}^{u_{i+1}} du \left( A''_i(u) \right)^2  & = & 12 \frac{ (\Delta_i^{i+1})^2 }{ \Delta_{i-2}^{i+1}\Delta_{i-1}^{i+1}}
\\        \int_{u_i}^{u_{i+1}} du  A''_i(u) B''_i(u)         & = &
