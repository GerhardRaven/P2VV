...
\begin{eqnarray}
   ...
   {\overline{\cal A}}_i(t) &=& e^{-t/2\tau} \left[  E_+(t) + \frac{1}{\eta_i\lambda} E_-(t)  \right] \eta_i a_i    \\
                            &=&     e^{-t/2\tau} \left[\eta_i E_+(t) + \frac{D-iS}{1-C} E_-(t)  \right] a_i   
\end{eqnarray}
where we have used that, by definition, $\overline{a}_i = \eta_i a_i$. The above can be
...
\\   {\overline{\cal A}}_i(t) &=&    \frac{e^{-t/2\tau}}{1-C} \left[ (1-C)E_+(t) +\eta_i (D-iS) E_-(t)  \right] \eta_i a_i  
%\\                           &=&     \frac{e^{-t/2\tau}}{1-C^2} \left[ (1-C^2)E_+(t)+\eta_i(D+iS)(1+C)E_-(t) \right] a_i
\end{eqnarray}
where we have used the properties of $\eta_i$ shown in (\ref{eq:etadef}).
...
As a result, the expressions for $\overline{b}_\mathrm{tag}$ are related to $b_\mathrm{tag}$ by the following substitution:
\begin{equation}
