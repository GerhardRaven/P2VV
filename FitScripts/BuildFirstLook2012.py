...
#2012
mcfilename =  '/data/bfys/dveijk/DataJpsiPhi/2012/NewMC.root'
...
ws.factory("{rz2[0.601,0.,1.],rperp2[0.16,0.,1.],rs2[0.,0.,1.]}")
...
except : from compatibility import product
...
#fulldata = ws['data'].reduce(RooFit.CutRange('unbiased'))
...
###########################
# Mass only PDF
ws.factory("SUM::m_pdf(Nsig_UB[1000,0,50000]*m_sig,Nbkg_UB[1000,0,20000]*m_bkg)")
ws['m_pdf'].fitTo(ws['data'],RooFit.NumCPU(8),RooFit.Extended(True),RooFit.Minos(False))

canvas = TCanvas()
mframe = ws['m'].frame()
ws['data'].plotOn(mframe)
ws['m_pdf'].plotOn(mframe)
ws['m_pdf'].plotOn(mframe,RooFit.Components('m_sig'),RooFit.LineColor(kGreen),RooFit.LineStyle(kDashed))
ws['m_pdf'].plotOn(mframe,RooFit.Components('m_bkg'),RooFit.LineColor(kRed),RooFit.LineStyle(kDashed))
mframe.Draw()
...
# Bkg PDF
ws.factory("PROD::bkg_pdf( m_bkg, t_bkg_UB, ang_bkg)")

ws.factory("SUM::pdf_ext(Nsig_UB*sig_pdf,Nbkg_UB*bkg_pdf)")
...
wsfile = TFile('FirstLookWS.root','RECREATE')
ws.Write()
wsfile.Close()
