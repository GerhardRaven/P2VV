...
parser = OptionParser()
parser.add_option("-v", type="string", dest="varname1")
parser.add_option("-w", type="string", dest="varname2")
parser.add_option("-x", type="int", dest="xstep")
parser.add_option("-y", type="int", dest="ystep")
parser.add_option("-n", type="int", dest="npoints")
parser.add_option("-a", type="int", dest="acceptance")
(options, args) = parser.parse_args()

_varname1 = options.varname1
_varname2 = options.varname2
_xstep = options.xstep
_ystep = options.ystep
_npoints = options.npoints
_acceptance = options.acceptance

print 'varname1 = ', _varname1
print 'varname2 = ', _varname2
print 'xstep =', _xstep
print 'ystep =', _ystep
...
print "before = ", obj._ws[_varname1].Print()
...
print "after = ", obj._ws[_varname1].Print()
...
### set parameters ###
######################
ypar = obj._ws[_varname1]
xpar = obj._ws[_varname2]
...
def determineconstval(nsteps,xstep,ystep):
    ymin = ypar.getMin()
    ymax = ypar.getMax()
    ystepsize = (ymax-ymin)/(nsteps)
    yval = ymin + 0.5*ystepsize+ystep*ystepsize

    xmin = xpar.getMin()
    xmax = xpar.getMax()
    xstepsize = (xmax-xmin)/(nsteps)
    xval = xmin + 0.5*xstepsize+xstep*xstepsize

    print 'Number of steps is %s, currently at ystep %s, setting value of yparameter %s to %s'%(nsteps,ystep,ypar.GetName(),yval)
    print 'Number of steps is %s, currently at xstep %s, setting value of xparameter %s to %s'%(nsteps,xstep,xpar.GetName(),xval)
    ypar.setVal(yval)
    ypar.setConstant()
    xpar.setVal(xval)
    xpar.setConstant()
...
determineconstval(_npoints,_xstep,_ystep)
...
def writevalues(par1,par2,rfr_before,rfr_after,xstep,ystep,filePath = 'llValues') :
    ...
    cont = '# %s: ll values\n' % fileName\
         ...
         + '# {0:<15} {1:<15} {2:<15} {3:<15} {4:<15} {5:<15}\n'\
               .format('xstep','ystep','xvarval','yvarval','original LL','fitted LL')\
         + '# -----------------------------------------------------------------------------------------------------------\n'

    cont += '  {0:<15.9g} {1:<15.9g} {2:<15.9g} {3:<15.9g} {4:<15.9g} {5:<15.9g}\n'.format(xstep,ystep,par1.getVal(),par2.getVal(),rfr_before.minNll(),rfr_after.minNll())
...
writevalues(xpar,ypar,fitresultpresent,fitresultafter,_xstep,_ystep,'%s_%s_LL.txt'%(_varname1,_varname2))
