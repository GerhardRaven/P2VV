...
iTag_os = Category( 'tagdecision_os', Title = 'initial state flavour tag OS', Observable = True, States = { 'B': +1, 'Bbar': -1} )
...
#################################
### Adjust sweighted datasets ###
#################################
from ROOT import RooFormulaVar
sigdata_cut = sigdata.reduce(RooFormulaVar("sigcut","sigcut","@0!=0",RooArgList(iTag_os._var)))
bkgdata_cut = bkgdata.reduce(RooFormulaVar("bkgcut","bkgcut","@0!=0",RooArgList(iTag_os._var)))

########################
### Build omega PDFs ###
########################
sig_omegaospdf = HistPdf( Name = 'sig_omegaospdf', Observables = (eta_os,), Data = sigdata_cut )
bkg_omegaospdf = HistPdf( Name = 'bkg_omegaospdf', Observables = (eta_os,), Data = bkgdata_cut )

etaframe = eta_os.frame()
#sigdata_cut.plotOn(etaframe)
#sig_omegaospdf.plotOn(etaframe)
bkgdata_cut.plotOn(etaframe)
bkg_omegaospdf.plotOn(etaframe)
etaframe.Draw()
...
background = Component('bkg'   , ( bkg_m.pdf(), bkg_t.pdf(), bkg_itagospdf, {eta_os: None}, bkg_sigmatpdf), Yield = ( nbkg, 0.9, 1.1*nbkg) )
...
signal         = Component('signal', ( sig_m.pdf(), sig_t_angles_iTag, sig_itagospdf, {eta_os: None}, sig_sigmatpdf), Yield = ( nsig, 0.9, 1.1*nsig) )
...
if timeacceptance:
    fitname = 'cfit_acc_taggedonly_noeta'
else:
    fitname = 'cfit_noacc_taggedonly_noeta'
