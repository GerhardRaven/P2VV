...
if reblinded:
    ...
    customizer = RooCustomizer(pdf,'reblinded')
...
if blinded:
    ...
    ws.factory("PROD::sig_pdf( m_sig, newpdf_reblinded)")
else:
...
angcorrpdf = buildEff_x_PDF(ws,'fourier_eff_full',pdf_ext,[ ( m.basis() , m.coefficient()/c_000 ) for m in moments if m.significance()>signif] )
ws.put(angcorrpdf)
...
#result = condpdf.fitTo(data,RooFit.NumCPU(8),RooFit.Extended(true),RooFit.Minos(false),RooFit.Save(true))
