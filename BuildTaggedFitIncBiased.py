####################################################################################################
####################################################################################################
# This script builds a root file with the workspace containing all information for the tagged fit of Note6.
#    * However, here we take the biased events into account.
...
#    * Blinding is handled in the UntaggedFit.py script when reading the workspace from the file generated by this script.
#    * Tagging systematics is handled in this script according to the methods in Note 3.

######################################################################################################
#    THIS SCRIPT WAS NEVER FINISHED SINCE WE STILL HAVE NO GOOD WAY TO TAKE PROPER TIME ACCEPTANCE INTO ACCOUNT USING ROOFIT!!!!
...
if useTransversityAngles:
    ws.factory("{ trcospsi[-1,1], trcostheta[-1,1], trphi[%f,%f], t[-2,20.], tagdecision[Bs_Jpsiphi=+1,Bsbar_Jpsiphi=-1,untagged=0], triggeredByUnbiasedHlt1AndHlt2[Unbiased=+1,NotUnbiased=0],triggeredByBiasedHlt1AndHlt2[Biased=+1,NotBiased=0]}"%(-pi,pi))
    ws.defineSet("transversityangles","trcospsi,trcostheta,trphi")
    angles = ws.set('transversityangles')

else:
    ws.factory("{ helcosthetaK[-1,1], helcosthetaL[-1,1], helphi[%f,%f], t[-2,20.], tagdecision[Bs_Jpsiphi=+1,Bsbar_Jpsiphi=-1,untagged=0], triggeredByUnbiasedHlt1AndHlt2[Unbiased=+1,NotUnbiased=0],triggeredByBiasedHlt1AndHlt2[Biased=+1,NotBiased=0]}"%(-pi,pi))
...
if useTransversityAngles:
    ws.defineSet("observables","m,t,trcospsi,trcostheta,trphi,tagdecision,triggeredByUnbiasedHlt1AndHlt2,triggeredByBiasedHlt1AndHlt2")
else:
    ws.defineSet("observables","m,t,helcosthetaL,helcosthetaK,helphi,tagdecision,triggeredByUnbiasedHlt1AndHlt2,triggeredByBiasedHlt1AndHlt2")
...
triggeredByUnbiasedHlt1AndHlt2 = ws.cat('triggeredByUnbiasedHlt1AndHlt2')
triggeredByBiasedHlt1AndHlt2 = ws.cat('triggeredByBiasedHlt1AndHlt2')
...
datafile = TFile('/data/bfys/dveijk/Data/Bs2JpsiPhiForTaggedFitAll.root')
...
print 'Summary tagdecision'
data.table(tagdecision).Print('v')
print 'TRIGGER SUMMARY FOR ALL EVENTS'
data.table(triggeredByUnbiasedHlt1AndHlt2).Print('v')
data.table(triggeredByBiasedHlt1AndHlt2).Print('v')

############
# Make SuperCategory from (triggeredByUnbiasedHlt1AndHlt2,triggeredByBiasedHlt1AndHlt2)
############
TypeCat = RooSuperCategory('TypeCat','TypeCat',RooArgSet(triggeredByUnbiasedHlt1AndHlt2,triggeredByBiasedHlt1AndHlt2))
data.table(TypeCat).Print('v')
data.table(TypeCat,'(tagdecision==-1 || tagdecision==1)').Print('v')
...
#########################
# Taking into account proper time acceptance of biased and unbiased events
#########################
